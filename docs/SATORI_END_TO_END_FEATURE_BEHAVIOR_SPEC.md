...
- Observability: JSON envelopes (`status`, `hints`, `warnings`, `freshnessDecision`, `freshnessSummary`), search telemetry `response_bytes`, and deterministic source-built debug projections selected by `debugMode=summary|ranking|freshness|full`; each mode constructs only its allowlisted fields, and existing `debug:true` remains an alias for `full`. Freshness/full diagnostics include proof mode, the cache invalidation reason, and counters for cache lookups/hits, cold readiness checks, warm receipt revalidations, and exact payload recounts.
