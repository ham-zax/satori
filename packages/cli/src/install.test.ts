...
test("managed offline launchers attach independent sessions to one shared runtime host", {
    ...
    try {
        ...
        assert.equal((await first.listTools()).tools.length, 8);
        assert.equal((await second.listTools()).tools.length, 8);
        ...
        assert.equal((await replacement.listTools()).tools.length, 8);
        assert.equal((await replacementSecond.listTools()).tools.length, 8);
