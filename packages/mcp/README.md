...
| `architecture_overview` | Summarize bounded Publication architecture evidence as logical areas, cross-area CALLS/IMPORTS boundaries, call hotspots, and coverage/filtering counts. Use `scope="runtime"` for product-oriented structure or `scope="all"` for the full published navigation corpus. |
