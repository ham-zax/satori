...
- Ready entries publish a non-unknown `symbolQuality` only when completion proof binds the exact current sealed navigation generation. Unbound, degraded, orphan, or mismatched local seals render `symbolQuality=unknown` because this compact projection cannot carry a separate evidence qualification.
...
- Completion proof distinguishes accepted-policy drift from malformed marker evidence. Internal `outcome:"policy_incompatible", reason:"runtime_policy_incompatible"` maps to the existing public `status:"requires_reindex", reason:"requires_reindex"` contract for semantic and navigation access. Completion validation refreshes repository profile inputs and requires the same durable v3 policy-bound collection accepted by active search. An unbound, missing, markerless, v1, v2, pre-seal, or mixed-version tuple cannot prove readiness and is not mutated or upgraded by validation.
