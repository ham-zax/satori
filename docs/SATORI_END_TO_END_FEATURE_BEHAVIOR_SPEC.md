...
- Read readiness is receipt-bound. A cached ready state carries the proven collection, exact marker identity/run ID, policy document digest/hash, payload count, navigation generation, manifest hashes, and canonical navigation-seal digest, plus the exact prepared-read observation under which revalidation succeeded. Every warm reuse rechecks collection existence, reads and compares one exact completion marker, verifies policy/profile and the immutable navigation generation seal, and requires that prepared observation to remain unchanged through freshness. Exact payload count is reused only while that sealed receipt remains identical. Cold proof and repair fully validate the referenced navigation artifacts and recompute the seal from them; warm observation reads `current.json`, `seal.json`, and stable tokens for the symbol/relationship directories so direct artifact loss invalidates cached readiness. Pre-seal navigation generations preserve vector proof but expose navigation as incompatible/unverified until `manage_index repair` republishes sealed local navigation.
- Reindex rollback never republishes an older generation through the current seal-required forward API. Before mutation it captures the exact durable policy document and navigation pointer bytes. Restoration is fenced by the live mutation lease when present and always compare-and-swaps against the captured candidate authority, then uses an fsynced two-file transaction journal so startup can complete an interrupted policy/pointer swap. This preserves a pre-seal vector generation as repairable without permitting stale workers or new forward publication to create an unsealed authority.
