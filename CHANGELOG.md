...
### Changed
- Installer preflight now verifies the exact managed MCP package name/version, starts every backend's candidate to prove MCP initialization and the canonical tool surface, loads LanceDB from that candidate, never overwrites an active runtime directory during reinstall, removes rejected new candidates, preserves managed local paths across blank shell forwarding, and fails closed on malformed managed runtime identity. Package tags and ranges are resolved again instead of silently reusing stale bytes.
- Vector-stack evaluation receipts now bind observations to the published collection and completion-marker/policy identity in addition to root, generation, and runtime fingerprint.
- **Breaking:** `@zokizuan/satori-core` embedding implementations now expose immutable `embedQuery`, `embedDocuments`, and `getIdentity` contracts. Custom implementations must return a stable provider/model/dimension/artifact/normalization identity; Core captures that identity when the embedding is installed and fails closed if it drifts before embedding work or publication. Mutable Voyage input-role state is removed.
- **Breaking:** `@zokizuan/satori-core` now has a restricted package export surface: the maintained public entry points are `.`, `./lancedb`, and `./package.json`; undeclared package subpath imports are no longer supported.
- Core's vector-store port now accepts backend-neutral indexed projections and separately typed control records. Custom adapters must persist supplied projections without rebuilding them and implement the control-record boundary independently from searchable documents.
...
- **Breaking:** exact-symbol `read_file` requests now require top-level `mode`, `open_symbol.contractVersion: 2`, exactly one symbol identity, and exactly one bounded context or continuation operation. Both modes return the same structured `symbol_context` transport; legacy unversioned full-span exact opens are rejected. Ordinary reads are unchanged. Direct-span opens remain unversioned but use the frozen canonical `startLine`/`endLine` field spelling; the former `start_line`/`end_line` direct-span shape is rejected.
