...
const TOOL_SUMMARIES: Readonly<Record<string, string>> = {
    architecture_overview: 'Summarize bounded Publication architecture evidence as logical areas, scoped cross-area CALLS/IMPORTS boundaries, call hotspots, structural graph-root entry candidates, area-level cycles, and observed relationship construct coverage. Optional subtree/exclusions apply consistently across the structural evidence.',
    detect_changes: 'Map a Git diff to current indexed symbol seeds and bounded inbound CALLS. Confirmed callers are labeled direct/transitive with causal paths and area aggregation; uncertain semantic references stay separate, and completeness reasons disclose traversal limits.',
    ...
    call_graph: 'Inspect admitted CALLS plus persisted semantic reference evidence. When inbound CALLS are empty, exact published-source occurrences are returned separately as observational sourceReferences before any optional ranked discovery hint.',
    find_references: 'Find ranking-independent exact textual occurrences of one canonical symbol across validated published source, with exact spans, owning symbols when available, deterministic path scope, and honest complete/partial coverage. Textual matches are observational only.',
